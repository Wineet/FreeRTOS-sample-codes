...
/* Library includes. */
//#include "LPC17xx.h"				//commented 24/04/16
#include "board.h"					//added 24/04/16
...
//void vSoftwareInterruptHandler( void );		//commented 24/04/16
//void WDT_IRQHandler(void);						//added 24/04/16
...
int main( void )
{
  ...
  //  xCountingSemaphore = xSemaphoreCreateCounting( 2, 0 );
    xQueueHandle1 = xQueueCreate(3,sizeof(int));
    ...
    //	prvSetupSoftwareInterrupt();
		...
		xTaskCreate( vProducerTask, "Producer", 240, NULL, 2, NULL );

		/* Create the task that will periodically generate a software interrupt.
		This is created with a priority below the handler task to ensure it will
		get preempted each time the handler task exist the Blocked state. */
		xTaskCreate( vConsumerTask, "Consumer", 240, NULL, 2, NULL );
...
static int volatile j=0;
static int volatile p =10;
static int volatile k;
static void vProducerTask( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		i = (LPC_GPIO2->PIN & (1<<12));
		if(i==0)
		{
			//xSemaphoreGive( xCountingSemaphore );              //counting semaphore increment
			xQueueSendToBack(xQueueHandle1,&p,4);
			++p;		/*to send some different data for another led*/
			//j=1;
...
static void vConsumerTask( void *pvParameters )
{
	...
	for( ;; )
	{
		//if(xSemaphoreTake( xCountingSemaphore, 250 ) == pdPASS)
		if(xQueueReceive(xQueueHandle1,&k,4) == pdPASS)
		{
	...
	//	vPrintString( "Periodic task - Interrupt generated.\n\n" );
		vTaskDelayUntil(&xLastWakeTime, ( 1002 / portTICK_RATE_MS ));
	}
}
/*-----------------------------------------------------------*/
