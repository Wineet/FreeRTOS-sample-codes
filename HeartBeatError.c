...
int main( void )
{
	...
	xTaskCreate(	vHeartBeat,		/* Pointer to the function that implements the task. */
...
void vHeartBeat( void *pvParameters )
{
	...
	for( ;; )
	{
		...
		{
		case 0:
			vTaskDelayUntil( &xLastWakeTime, (portTickType)250 );
			i = (LPC_GPIO2->PIN & (1<<9));
			LPC_GPIO2->PIN = (i ^ (1<<9));
		...
		case 1:
			vTaskDelayUntil( &xLastWakeTime, (portTickType)450 );
			i = (LPC_GPIO2->PIN & (1<<9));
			LPC_GPIO2->PIN = (i ^ (1<<9));
		...
		case 2:
			vTaskDelayUntil( &xLastWakeTime, (portTickType)550 );
			i = (LPC_GPIO2->PIN & (1<<9));
			LPC_GPIO2->PIN = (i ^ (1<<9));
